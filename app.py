...
from config.settings import UI_CONFIG, TEST_QUERIES, REPORTS_DIR
...
def render_header():
    ...
    st.markdown('<h1 class="main-header">💊 Pharma Agentic AI</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; color: #666;">Portfolio Planning Assistant - Drug Repurposing Intelligence</p>', unsafe_allow_html=True)
    st.markdown("---")
...
def render_response():
    ...
    st.markdown(f'<div class="query-box">{response["query"]}</div>', unsafe_allow_html=True)
    ...
    with st.expander("🔍 Query Analysis", expanded=False):
        ...
        with col1:
            ...
            st.write("**Entities:**", ', '.join(parsed.get('entities', [])) or 'None detected')
        with col2:
...
def render_response():
    ...
    with st.expander("🔍 Query Analysis", expanded=False):
        ...
        with col1:
            st.write("**Intent:**", parsed.get('intent', 'N/A'))
            st.write("**Entities:**", ', '.join(parsed.get('entities', {}).get('drugs', [])) or 'None detected')
        with col2:
            st.write("**Required Agents:**", ', '.join(parsed.get('required_agents', [])))
            st.write("**Keywords:**", ', '.join(parsed.get('keywords', [])[:5]))
        ...
        if st.button("📄 Generate PDF Report"):
            with st.spinner("Generating PDF report..."):
                ...
                result = report_agent.generate_report(
                    ...
                    agent_responses=st.session_state.current_response['agent_responses'],
...
def render_query_history():
    ...
    for i, item in enumerate(recent_queries):
        with st.expander(f"🕐 {item['timestamp']} - {item['query'][:60]}..."):
            ...
            st.write(f"**Intent:** {item['result']['parsed_query'].get('intent', 'N/A')}")
