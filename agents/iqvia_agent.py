...
class IQVIAAgent:
    ...
    def analyze_market(
        ...
        results["competitive_landscape"] = self._analyze_competition(drugs, therapeutic_areas)
        
        if self.verbose:
            print(f"✓ Analysis complete - {len(results['drug_analyses'])} drugs, {len(results['therapeutic_area_analyses'])} therapeutic areas")
        
        return results
    
    def _analyze_single_drug(self, drug_name: str, time_scope: str) -> Optional[Dict[str, Any]]:
        """
        ...
        sales_growth = latest_data['sales_usd_million'] - oldest_data['sales_usd_million']
        sales_growth_pct = (sales_growth / oldest_data['sales_usd_million']) * 100 if oldest_data['sales_usd_million'] > 0 else 0
    ...
    def _analyze_therapeutic_area(self, therapeutic_area: str, time_scope: str) -> Dict[str, Any]:
        """
        ...
        drugs_in_area = self.data_fetcher.get_drugs_by_therapeutic_area(therapeutic_area)
        ...
        for drug in drugs_in_area:
            ...
            if market_data and 'data_points' in market_data:
                ...
                if data_points:
                    latest = sorted(data_points, key=lambda x: x['year'], reverse=True)[0]
        ...
        for drug_summary in drug_summaries:
            drug_summary['market_share_percent'] = round(
                (drug_summary['sales_usd_million'] / total_sales * 100), 2
        ...
        analysis = {
            ...
            "average_drug_sales_usd_million": round(total_sales / len(drugs_in_area), 2) if drugs_in_area else 0,
            "top_drugs": drug_summaries[:5],  # Top 5 drugs
            "market_leaders": [d['drug_name'] for d in drug_summaries[:3]],
            "competitive_intensity": self._calculate_competitive_intensity(drug_summaries)
        }
    ...
    def _analyze_trend(self, data_points: List[Dict]) -> str:
        ...
        if len(sorted_points) >= 2:
            recent_growth = sorted_points[-1]['sales_usd_million'] - sorted_points[-2]['sales_usd_million']
            if recent_growth > 0:
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        if results['drug_analyses']:
            ...
            total_sales = sum(d['market_metrics']['current_sales_usd_million'] for d in results['drug_analyses'])
            avg_cagr = sum(d['market_metrics'].get('cagr_percent', 0) for d in results['drug_analyses']) / num_drugs if num_drugs > 0 else 0
        ...
        return " ".join(summary_parts) if summary_parts else "No market data available for analysis."
    ...
    def _extract_market_insights(self, results: Dict[str, Any]) -> List[str]:
        ...
        high_growth_drugs = [
            d for d in results['drug_analyses']
        ...
        if declining_drugs:
            insights.append(f"{len(declining_drugs)} drug(s) showing declining trends - potential repurposing candidates")
        ...
        if large_markets:
            insights.append(f"{len(large_markets)} blockbuster drug(s) with sales >$200M annually")
        ...
        if therapeutic_areas:
            for area in therapeutic_areas:
                ...
                if len(drugs_in_area) >= 3:
                    competition["market_leaders"].extend([d['name'] for d in drugs_in_area[:3]])
    ...
    def get_market_trends(
        ...
        data_points = sorted(market_data['data_points'], key=lambda x: x['year'], reverse=True)[:years]
    ...
    def compare_drugs(self, drug_names: List[str]) -> Dict[str, Any]:
        ...
        for drug_name in drug_names:
            ...
            if analysis:
                comparisons.append({
                    "drug_name": drug_name,
                    "sales_usd_million": analysis['market_metrics']['current_sales_usd_million'],
        ...
        return {
            ...
            "highest_growth": max(comparisons, key=lambda x: x.get('cagr_percent', 0))['drug_name'] if comparisons else None
        }
