...
from utils.data_fetchers import OpenFDAFetcher, PubMedFetcher
...
class WebIntelligenceAgent:
    ...
    def search_literature(
        ...
        if keywords:
            for keyword in keywords:
                ...
                if publications:
                    ...
                    if self.verbose:
                        print(
                            f"  Found {len(publications)} publications for: {keyword}"
                        )
        ...
        if drugs:
            for drug in drugs:
                # PubMed search
                publications = self.pubmed_fetcher.search_publications(
                    drug, max_results=max_results
                )
                if publications:
                    all_publications.extend(publications)
                    if self.verbose:
                        print(
                            f"  Found {len(publications)} publications for drug: {drug}"
                        )
        ...
        if results["fda_information"]:
            ...
            for drug, info in results["fda_information"].items():
                if info.get("warnings") and info["warnings"] != "N/A":
                    findings.append(
                        f"{drug}: FDA warnings available - review recommended"
                    )

        # Publication volume
        if results["publications_by_topic"]:
            most_researched = max(
                results["publications_by_topic"].items(), key=lambda x: x[1]["count"]
            )
            findings.append(
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        if results["research_trends"]:
            summary_parts.append(
                f"{len(results['research_trends'])} research trend(s) identified."
            )

        return " ".join(summary_parts)

    def get_drug_guidelines(
        self, drug_name: str, indication: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def search_repurposing_evidence(
        self, drug_name: str, new_indication: str
    ) -> Dict[str, Any]:
        """
        ...
        evidence_strength = (
            ...
            else "Moderate" if len(publications) >= 5 else "Limited"
        )
        ...
        return {
            ...
            "repurposing_potential": self._assess_repurposing_potential(
                len(publications), new_indication
            ),
            "recommendation": self._generate_repurposing_recommendation(
                evidence_strength, len(publications)
            ),
        }

    def _assess_repurposing_potential(
        self, publication_count: int, indication: str
    ) -> str:
        """
    ...
    def _generate_repurposing_recommendation(
        self, evidence_strength: str, pub_count: int
    ) -> str:
        """
            ...
            comparisons.append(
                {
                    ...
                    "research_activity": (
                        ...
                        else "Moderate" if len(publications) >= 15 else "Low"
                    ),
                }
            )
    ...
    def _generate_comparison_summary(self, comparisons: List[Dict]) -> str:
        ...
        return (
            f"Compared {len(comparisons)} drugs with {total_pubs} total publications. "
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    comparison = agent.compare_research_activity(
