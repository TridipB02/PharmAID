...
class ClinicalTrialsAgent:
    ...
    def search_trials(
        ...
        if drugs:
            for drug in drugs:
                trials = self.fetcher.search_trials(
                    intervention=drug, max_results=max_results
                )
                if trials:
        ...
        if diseases:
            for disease in diseases:
                trials = self.fetcher.search_trials(
                    condition=disease, max_results=max_results
                )
                if trials:
        ...
        if therapeutic_areas:
            for area in therapeutic_areas:
                trials = self.fetcher.search_trials(
                    condition=area, max_results=max_results
                )
                if trials:
                    all_trials.extend(trials)
        ...
        if results["detailed_trials"]:
            results["phase_distribution"] = self._analyze_phases(
                results["detailed_trials"]
            )
            results["sponsor_analysis"] = self._analyze_sponsors(
                results["detailed_trials"]
            )
            ...
            results["repurposing_opportunities"] = (
                self._identify_repurposing_opportunities(results, drugs)
            )
        ...
        if self.verbose:
            print(
                f"✓ Search complete - {results['total_trials_found']} unique trials found"
            )
    ...
    def _analyze_phases(self, trials: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_phase": (
                phase_counts.most_common(1)[0][0] if phase_counts else "N/A"
            ),
            "advanced_trials_count": sum(
                1 for p in phases if "Phase 3" in p or "Phase 4" in p
            ),
        }
    ...
    def _analyze_sponsors(self, trials: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "industry_vs_academic_ratio": (
                round(len(industry_sponsors) / len(academic_sponsors), 2)
                if academic_sponsors
                else "N/A"
            ),
        }
    ...
    def _analyze_status(self, trials: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            + status_counts.get("Active, not recruiting", 0),
            "completed_trials": status_counts.get("Completed", 0),
            "ongoing_percentage": (
                round(
                    (
                        (
                            status_counts.get("Recruiting", 0)
                            + status_counts.get("Active, not recruiting", 0)
                        )
                        / total
                        * 100
    ...
    def _extract_key_findings(self, results: Dict[str, Any]) -> List[str]:
        ...
        if results["phase_distribution"]:
            ...
            findings.append(
                f"Most trials in {most_common} with {advanced} in advanced stages (Phase 3/4)"
            )

        # Sponsor findings
        if results["sponsor_analysis"]:
            top_sponsor = (
                results["sponsor_analysis"]["top_sponsors"][0]
                if results["sponsor_analysis"]["top_sponsors"]
                else None
            )
            if top_sponsor:
                findings.append(
                    f"Leading sponsor: {top_sponsor['sponsor']} with {top_sponsor['trial_count']} trials"
                )
        ...
        if results["status_summary"]:
            ...
            findings.append(
                f"{active} active/recruiting trials representing strong ongoing research interest"
            )

        return findings

    def _identify_repurposing_opportunities(
        self, results: Dict[str, Any], drugs: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
                    ...
                    opportunities.append(
                        {
                            ...
                            "repurposing_potential": (
                                "High" if len(conditions) >= 3 else "Moderate"
                            ),
    ...
    def analyze_pipeline(self, intervention: str) -> Dict[str, Any]:
        """
        ...
        phase_counts = Counter(
            trial["phase"] for trial in trials if trial["phase"] != "N/A"
        )
        ...
        return {
            ...
            "active_trials": sum(
                1
                for t in trials
                if t["status"] in ["Recruiting", "Active, not recruiting"]
            ),
    ...
    def _assess_pipeline_health(self, trials: List[Dict]) -> str:
        ...
        active_count = sum(
            1 for t in trials if t["status"] in ["Recruiting", "Active, not recruiting"]
        )
        ...
        has_advanced = any(
            "Phase 3" in t["phase"] or "Phase 4" in t["phase"] for t in trials
        )
    ...
    def compare_pipelines(self, interventions: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_active_pipeline": (
                comparisons[0]["intervention"] if comparisons else None
            ),
    ...
    def _generate_comparison_summary(self, comparisons: List[Dict]) -> str:
        ...
        return (
            f"Compared {len(comparisons)} pipelines with {total_trials} total trials. "
