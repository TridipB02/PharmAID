...
class InternalKnowledgeAgent:
    ...
    def search_internal(
        ...
        if not documents:
            results["summary"] = "No internal documents available in the knowledge base."
            if self.verbose:
        ...
        if matching_docs:
            ...
            results["relevant_excerpts"] = self._extract_excerpts(matching_docs, keywords)
        
        # Generate summary
        results["summary"] = self._generate_summary(results)
        
        if self.verbose:
            print(f"✓ Search complete - {results['total_documents_found']} documents found")
    ...
    def _extract_insights(
        ...
        if doc_types:
            type_summary = ", ".join([f"{count} {ext}" for ext, count in doc_types.items()])
            insights.append(f"Document types: {type_summary}")
        
        # Recent documents
        if documents:
            recent_docs = sorted(documents, key=lambda x: x['modified'], reverse=True)[:3]
                ...
                try:
                    with open(doc['path'], 'r', encoding='utf-8') as f:
                        ...
                        excerpts.append({
                            ...
                            "relevance": "High" if keywords and any(kw.lower() in content.lower() for kw in keywords) else "Medium"
                        })
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        if results['key_insights']:
            summary_parts.append(f"{len(results['key_insights'])} key insight(s) extracted.")
        ...
        if file_path.suffix.lower() in ['.txt', '.md']:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    summary['preview'] = content[:500] + "..." if len(content) > 500 else content
                    summary['word_count'] = len(content.split())
            except Exception as e:
                summary['preview'] = f"Could not read content: {e}"
        else:
            summary['preview'] = f"[{file_path.suffix.upper()} file - binary or requires special parser]"
