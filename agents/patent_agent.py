...
class PatentAgent:
    ...
    def search_patents(
        ...
        if results["detailed_patents"]:
            results["expiry_timeline"] = self._analyze_expiry_timeline(results["detailed_patents"])
            results["fto_assessment"] = self._assess_fto(results["detailed_patents"], drugs)
            results["competitive_landscape"] = self._analyze_competitive_landscape(results["detailed_patents"])
        ...
        if self.verbose:
            print(f"✓ Search complete - {results['total_patents_found']} unique patents found")
    ...
    def _analyze_expiry_timeline(self, patents: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "opportunity_window": "High" if len(expiring_soon) > 0 else "Medium" if len(expiring_medium) > 0 else "Low"
        }
    
    def _assess_fto(self, patents: List[Dict], drugs: List[str] = None) -> Dict[str, Any]:
        """
        Assess Freedom to Operate (FTO) risks
        ...
        if active_count == 0:
            ...
            risk_level = "High"
            risk_description = f"{active_count} active patents - significant FTO concerns"
        ...
        return {
            ...
            "top_patent_holders": [
                ...
                for assignee in sorted(set(assignees), key=lambda x: assignees.count(x), reverse=True)[:5]
            ],
            "blocking_patents": [
                {
    ...
    def _analyze_competitive_landscape(self, patents: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_leader": assignee_counts.most_common(1)[0][0] if assignee_counts else "Unknown",
            "filing_trend": {
                "recent_filings_5yr": recent_filings,
                "annual_breakdown": dict(sorted(year_counts.items(), reverse=True)[:10]),
                "innovation_momentum": "High" if recent_filings > len(patents) * 0.3 else "Moderate" if recent_filings > 0 else "Low"
            },
    ...
    def _calculate_concentration(self, assignee_counts: Counter) -> str:
        ...
        top_3_share = sum(count for _, count in assignee_counts.most_common(3)) / total if total > 0 else 0
    ...
    def _identify_white_space(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if results['competitive_landscape']['filing_trend']['innovation_momentum'] == "Low":
            opportunities.append({
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> str:
        ...
        if results['expiry_timeline']:
            ...
            if expiring_soon > 0:
                summary_parts.append(f"{expiring_soon} patent(s) expiring within 2 years - window of opportunity.")
        ...
        if results['white_space_opportunities']:
            summary_parts.append(f"{len(results['white_space_opportunities'])} white space opportunity(ies) identified.")
    ...
    def compare_patent_positions(self, drugs: List[str]) -> Dict[str, Any]:
        ...
        for drug in drugs:
            ...
            comparisons.append({
                ...
                "expiring_soon": result['expiry_timeline'].get('expiring_soon_count', 0),
        ...
        comparisons.sort(
            key=lambda x: x['expiring_soon'] + x['opportunities'], 
            reverse=True
        )
    ...
    def _generate_comparison_summary(self, comparisons: List[Dict]) -> str:
        ...
        return f"Compared {len(comparisons)} drugs with {total_patents} total patents. " \
...
if __name__ == "__main__":
    ...
    print("="*70)
    comparison = agent.compare_patent_positions(["Metformin", "Atorvastatin", "Ibuprofen"])
